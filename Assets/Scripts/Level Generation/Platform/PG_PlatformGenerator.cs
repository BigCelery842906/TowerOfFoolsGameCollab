...
public class PG_PlatformGenerator : MonoBehaviour
{
    ...
    public int m_failedBonusPlatformSpawnAttempts = 50;
    ...
    List<GameObject> m_criticalPathPlatforms;
    List<GameObject> m_bonusPlatforms;
    ...
    public void Awake()
    {
        ...
        m_platformList = new();
        m_criticalPathPlatforms = new();
        m_bonusPlatforms = new();
    ...
    private IEnumerator SpawnBonusPlatforms()
    {
        ...
        while (platformsSpawned < m_numberOfBonusPlatforms && attempts < m_failedBonusPlatformSpawnAttempts)
        {
            attempts++;
            int platformSize;
            ...
            while (!placedAndValid && innerAttempts < MaxInnerAttempts)
            {
                ...
                if (!SpawnPlatformAtCoords(xPos, yPos, platformSize, 1, false))
                {
    ...
    void ZigZag(GameObject room)
    {
        ...
        SpawnPlatformAtCoords(xPointer, yPointer, 3, 1, true);
        while (!platformsFinished)
        {
            ...
            SpawnPlatformAtCoords(xPointer, yPointer, platformSize, 1, true);
    ...
    /// </summary>
    public bool SpawnPlatformAtCoords(int x, int y, int xSize, int ySize, bool criticalPath)
    {
        PG_GridMap roomGrid = m_currentRoom.GetComponent<PG_GridMap>();
        if (!roomGrid)
        {
        ...
        adjustedZPos.z += 0.75f;
        platformContainer.transform.position = adjustedZPos;
        m_platformUndoStack.Push(platformContainer);
        m_platformList.Add(platformContainer);
        if (criticalPath)
        {
            m_criticalPathPlatforms.Add(platformContainer);
        }
        else
        {
            m_bonusPlatforms.Add(platformContainer);
        }
        return true;

    }
    public void UndoPlatformPlacement(bool criticalPath)
    {
        ...
        if (criticalPath)
        {
            m_criticalPathPlatforms.RemoveAt(m_criticalPathPlatforms.Count - 1);
        }
        else
        {
            m_bonusPlatforms.RemoveAt(m_bonusPlatforms.Count - 1);
        }
        m_platformList.RemoveAt(m_platformList.Count - 1);
    ...
    public List<GameObject> GetBonusPlatforms()
    {
        return m_bonusPlatforms;
    }
